sta "C:\Users\Bon-Minh Lam\plsql_parser\scratch_pad\mistral-EBNF-simplifier.sql"
...
DECLARE 
    ...
    BEGIN
        FOR i IN 1..LENGTH(p_string) LOOP
            ...
            -- Track brackets
            ELSIF v_char = 'xxx[xxx' AND NOT v_in_quotes THEN
                v_in_brackets := v_in_brackets + 1;
            ELSIF v_char = 'xxx]xxx' AND NOT v_in_quotes THEN
                v_in_brackets := v_in_brackets - 1;
            -- Track parentheses
            ELSIF v_char = 'xx(xx' AND NOT v_in_quotes THEN
    ...
    BEGIN
        ...
        IF SUBSTR(p_expr, 1, 1) = 'xx[xx' AND SUBSTR(p_expr, -1, 1) = 'xx]xx' THEN
            v_inner := SUBSTR(p_expr, 2, LENGTH(p_expr) - 2);
...
END;
    ...
    type hash_type is table of number index by varchar2(10 char);
    chr_counter hash_type;
    v_code varchar2(1 char);
--    v_code NUMBER;
    v_chr_val NUMBER;
    v_ln_pos NUMBER := 0;
    v_col_pos NUMBER := 0;
    v_new_text VARCHAR2(30000 CHAR);
begin 
    for i in 1 .. length( qstr ) 
    loop 
        v_code := substr( qstr, i, 1)  ;
        IF v_code = chr(10) THEN 
            v_ln_pos := v_ln_pos;
            v_col_pos := 0;
        END IF;
        v_col_pos := v_col_pos + 1;
        BEGIN
            v_chr_val := ascii( v_code );
            IF v_chr_val not between 1 and 127 then 
                dbms_output.put_line( 'trouble at ln/col '||v_col_pos||'/'||v_col_pos ) ;
            end if;
        EXCEPTION 
            WHEN OTHERS THEN    
                dbms_output.put_line( 'trouble at ln/col '||v_col_pos||'/'||v_col_pos ) ;
        END ;
        if not chr_counter.exists( v_code ) 
        then 
            chr_counter( v_code ) := 1 ;
        else 
            chr_counter( v_code ) := chr_counter( v_code ) + 1  ;
        end if;
    end loop ;
    -- 
    -- print 
